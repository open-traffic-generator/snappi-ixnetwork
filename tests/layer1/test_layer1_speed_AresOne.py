...
@pytest.mark.parametrize(
    "speed",
    ["speed_50_gbps", "speed_100_gbps", "speed_200_gbps", "speed_400_gbps"],
)
...
def get_port_locations(location):
    ...
    for rg in range(1, 9):
        rg_fanout200g[rg] = list(
            range(rg_fanout200g_starting_port, rg_fanout200g_starting_port + 2)
        )
        rg_fanout200g_starting_port += 2

    for rg in range(1, 9):
        rg_fanout100g[rg] = list(
            range(rg_fanout100g_starting_port, rg_fanout100g_starting_port + 4)
        )
        rg_fanout100g_starting_port += 4

    for rg in range(1, 9):
        rg_fanout50g[rg] = list(
            range(rg_fanout50_starting_port, rg_fanout50_starting_port + 2)
        )
    ...
    port_400g = ";".join(location.split(";")[0:2]) + ";" + str(port_400g)
