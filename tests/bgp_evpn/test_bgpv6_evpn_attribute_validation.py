...
def test_bgpv6_evpn_validation(api, utils):
    ...
    tx_ip = tx_eth.ipv6_addresses.ipv6(name='tx_ip',
                                       address='2000::2',
                                       gateway='2000::1')[-1]
    ...
    tx_bgp_iface = (tx_bgp.ipv6_interfaces
                    .v6interface(ipv6_name=tx_ip.name)[-1])
    tx_bgp_peer = tx_bgp_iface.peers.v6peer(name="tx_eBGP",
                                            peer_address='2000::1',
                                            as_type='ebgp',
                                            as_number=100)[-1]
    ...
    tx_vtep_bgp_iface = (tx_vtep_bgp.ipv6_interfaces
                         .v6interface(ipv6_name=tx_l1.name)[-1])
    tx_vtep_bgp_peer = tx_vtep_bgp_iface.peers.v6peer(name="bgp1",
                                                      peer_address='2000::1',
                                                      as_type='ibgp',
                                                      as_number=101)[-1]
    ...
    tx_eth_seg.df_election.election_timer = (
        BGPV6_EVPN_ETH_SEGMENT["DfElectionTimer"])
    ...
    tx_eth_seg.advanced.multi_exit_discriminator = (
        BGPV6_EVPN_ETH_SEGMENT["MultiExitDiscriminator"])
    ...
    tx_evi_vxlan.route_distinguisher.rd_value = (
        str(BGPV6_EVPN_VXLAN["RdASNumber"]) + ":" + str(
            BGPV6_EVPN_VXLAN["RdEvi"]))
    tx_evi_vxlan.ad_label = BGPV6_EVPN_VXLAN["AdRouteLabel"]
    tx_evi_vxlan.pmsi_label = (
        BGPV6_EVPN_VXLAN["UpstreamDownstreamAssignedMplsLabel"])
    ...
    import_rt.rt_value = (
        BGPV6_EVPN_VXLAN_IMPORT_TARGET[
            "TargetAs4Number"] + ":" + BGPV6_EVPN_VXLAN_IMPORT_TARGET[
                "TargetAssignedNumber"])
...
def validate_config(api,
    ...
    bgps = (
        ...
        .DeviceGroup.find().Ipv6Loopback.find().BgpIpv6Peer.find())
    for bgp in bgps:
        assert bgp.EthernetSegmentsCountV6 == 1
        assert bgp.BgpEthernetSegmentV6.EvisCount == 1
        evis = bgp.BgpIPv6EvpnVXLAN.find()
        assert evis.Multiplier == 1

    bgp_eth_seg = bgps[0].BgpEthernetSegmentV6
    for attr in BGPV6_EVPN_ETH_SEGMENT:
        if attr in ["DfElectionTimer", "EsiLabel",
                    "MultiExitDiscriminator"]:
            assert BGPV6_EVPN_ETH_SEGMENT[attr] == int(
                (getattr(bgp_eth_seg, attr).Values)[0]
            )
        else:
            assert BGPV6_EVPN_ETH_SEGMENT[attr] == (
                (getattr(bgp_eth_seg, attr).Values)[0]
            )

    bgp_eth_seg_comm_list = (
        bgps[0].BgpEthernetSegmentV6.BgpCommunitiesList.find())
    for attr in BGPV6_EVPN_ETH_SEGMENT_COMMUNITIES_lIST:
        assert BGPV6_EVPN_ETH_SEGMENT_COMMUNITIES_lIST[attr] == (
            (getattr(bgp_eth_seg_comm_list, attr).Values)[0]
        )

    bgp_eth_seg_ext_comm_list = (
        bgps[0].BgpEthernetSegmentV6.BgpExtendedCommunitiesList.find())
    for attr in BGPV6_EVPN_ETH_SEGMENT_EXT_COMMUNITIES_lIST:
        assert BGPV6_EVPN_ETH_SEGMENT_EXT_COMMUNITIES_lIST[attr] == (
            (getattr(bgp_eth_seg_ext_comm_list, attr).Values)[0]
        )

    bgp_eth_seg_aspath_segments_list = (
        bgps[0].BgpEthernetSegmentV6.BgpAsPathSegmentList.find())
    for attr in BGPV6_EVPN_ETH_SEGMENT_ASPATH_SEGMENTS_lIST:
        assert BGPV6_EVPN_ETH_SEGMENT_ASPATH_SEGMENTS_lIST[attr] == (
            (getattr(bgp_eth_seg_aspath_segments_list, attr).Values)[0]
        )

    bgp_evpn_vxlan = bgps[0].BgpIPv6EvpnVXLAN.find()
    for attr in BGPV6_EVPN_VXLAN:
        assert BGPV6_EVPN_VXLAN[attr] == int(
            (getattr(bgp_evpn_vxlan, attr).Values)[0]
        )

    bgp_evpn_vxlan_export_target = (
        bgp_evpn_vxlan.BgpExportRouteTargetList.find())
    for attr in BGPV6_EVPN_VXLAN_EXPORT_TARGET:
        assert BGPV6_EVPN_VXLAN_EXPORT_TARGET[attr] == (
    ...
    for attr in BGPV6_EVPN_VXLAN_IMPORT_TARGET:
        assert BGPV6_EVPN_VXLAN_IMPORT_TARGET[attr] == (
    ...
    for attr in BGPV6_EVPN_VXLAN_L3_EXPORT_TARGET:
        assert BGPV6_EVPN_VXLAN_L3_EXPORT_TARGET[attr] == (
    ...
    for attr in BGPV6_EVPN_VXLAN_L3_IMPORT_TARGET:
        assert BGPV6_EVPN_VXLAN_L3_IMPORT_TARGET[attr] == (
            (getattr(bgp_evpn_vxlan_l3_import_target, attr).Values)[0]
        )

    bgp_eth_seg_vxlan_comm_list = (
        bgps[0].BgpIPv6EvpnVXLAN.find().BgpCommunitiesList.find())
    for attr in BGPV6_EVPN_ETH_SEGMENT_COMMUNITIES_lIST:
        assert BGPV6_EVPN_ETH_SEGMENT_COMMUNITIES_lIST[attr] == (
    ...
    bgp_eth_seg_vxlan_ext_comm_list = (
        bgps[0].BgpIPv6EvpnVXLAN.find().BgpExtendedCommunitiesList.find())
    for attr in BGPV6_EVPN_ETH_SEGMENT_EXT_COMMUNITIES_lIST:
        assert BGPV6_EVPN_ETH_SEGMENT_EXT_COMMUNITIES_lIST[attr] == (
    ...
    bgp_eth_seg_vxlan_aspath_segments_list = (
        bgps[0].BgpIPv6EvpnVXLAN.find().BgpAsPathSegmentList.find())
    for attr in BGPV6_EVPN_ETH_SEGMENT_ASPATH_SEGMENTS_lIST:
        assert BGPV6_EVPN_ETH_SEGMENT_ASPATH_SEGMENTS_lIST[attr] == (
