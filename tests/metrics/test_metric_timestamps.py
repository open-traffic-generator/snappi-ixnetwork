...
def test_metric_timestamps(api, utils, b2b_raw_config, tx_port, rx_port):
    """This is a test script to test timestamps metrics,
    metrics should be available only for the flows timestamps is enabled

    1. Create two flows f1&f2
    2. f1 with timestamps enabled
    3. f2 with timestamps disabled

    Validation:
    - Check stats and timestamps metrics are available only for f1
    """
    SIZE = 1024
    PACKETS = 1000
