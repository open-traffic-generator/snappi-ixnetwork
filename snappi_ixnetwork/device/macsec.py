...
class Macsec(Base):
    ...
    def _config_ethernet_interfaces(self, device):
        ...
        for ethernet_interface in ethernet_interfaces:
            ...
            if not self._is_valid(ethernet_name, ethernet_interface.get("secure_entity")):
                continue
    ...
    def _config_secy(self, device, ethernet_interface):
        ...
        if self.is_data_plane:
            if secy.data_plane.encapsulation.crypto_engine.choice == "encrypt_only":
                ...
                self._config_data_plane(device, ethernet_interface, ixn_staticmacsec)
    ...
    def _config_data_plane(self, device, ethernet_interface, ixn_staticmacsec):
        ...
        self._config_crypto_engine(device, ethernet_interface, ixn_staticmacsec)
    ...
    def _config_data_plane_tx(self, encapsulation, ixn_staticmacsec):
        ...
        if tx is None:
            return
        self.logger.debug("end_station %s include_sci %s" % (tx.end_station, tx.include_sci))
    ...
    def _config_crypto_engine(self, device, ethernet_interface, ixn_staticmacsec):
        ...
        if crypto_engine is None:
            return
        if crypto_engine.choice == "encrypt_only":
            self._config_crypto_engine_encrypt_only(device, ethernet_interface, ixn_staticmacsec)
    ...
    def _config_static_key_txsc(self, static_key, ixn_staticmacsec):
        ...
        txsc = txscs[0]
        self.configure_multivalues(txsc, ixn_staticmacsec, Macsec._STATIC_KEY_TXSC)
        tx_saks = txsc.saks
    ...
    def _config_static_key_rxsc(self, static_key, ixn_staticmacsec):
        ...
        rxsc = rxscs[0]
        self.configure_multivalues(rxsc, ixn_staticmacsec, Macsec._STATIC_KEY_RXSC)
        rx_saks = rxsc.saks
    ...
    def _config_crypto_engine_encrypt_only(self, device, ethernet_interface, ixn_staticmacsec):
        secy = ethernet_interface.get("secure_entity")
        self._config_crypto_engine_encrypt_only_tx_pn(secy, ixn_staticmacsec)
        self._config_crypto_engine_encrypt_only_traffic(device, ethernet_interface, ixn_staticmacsec)

    def _config_crypto_engine_encrypt_only_traffic(self, device, ethernet_interface, ixn_staticmacsec):
        ethernet_name = ethernet_interface.get("eth_name")
        secy = ethernet_interface.get("secure_entity")
