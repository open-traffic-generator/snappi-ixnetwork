        ...
        for device in self.api.snappi_config.devices:
            ...
            for ethernet in ethernets:
                if ethernet.get("connection") and ethernet.get("port_name"):
                    raise Exception(
                        "port_name and connection for ethernet configuration  \
                        cannot be passed together, use either connection  \
                        or port_name property. port_name is deprecated and \
        ...
        for ethernet_name in ethernet_names:
            ...
            for gateway_ip in gateway_ips:
                if gateway_ip not in arp_entries:
                    raise Exception(
                        "{} not found within current configured gateway ips"
                        .format(gateway_ip)
                    )
                if choice == "ipv4":
