...
class Lag(object):
    ...
    _LACP = {
        ...
        'lacpdu_timeout' : {
            'ixn_attr' : 'lacpduTimeout',
            'default' : '0'
        },
        'actor_activity' : {
            'ixn_attr' : 'lacpActivity',
            'default' : 'active'
        },
    }
    ...
    def config(self):
        ...
        self._lag_ports = {}
        self._lags_config = self._api.config.lags
    ...
    def _get_vports(self, ixn_vports, ports):
        ...
        for port in ports:
            if port.port_name not in ixn_vports:
                raise Exception("Port {0} is not available".format(
                    port.name))
    ...
    def _create_lags(self):
        ...
        """
        self._ixn_lag.find()
        existing_lags = [ixn_lag.Name
                         for ixn_lag in self._ixn_lag]
    ...
    def _ethernet_config(self):
        ...
        for name, ports in self._lag_ports.items():
            ...
            for eth_attr in Lag._ETHERNET:
                attr_values = self._configure_attribute(eth_attr, Lag._ETHERNET,
                                                        ports, 'ethernet')
                imports.append(self._set_multivalue(ether_xpath, attr_values.ixn_attribute,
                                                    attr_values.config_value))
            vlans = self._process_vlans(ports)
            if len(vlans) > 0:
                if len(vlans) > 0:
                    imports.append(self._set_multivalue(ether_xpath, 'enableVlans',
                                                    True))
                    imports.append({
                        "xpath": ether_xpath,
                        "vlanCount": len(vlans)
                ...
                for idx, vlan in enumerate(vlans):
                    for vlan_attr in Lag._VLAN:
                        attr_values = self._configure_attribute(vlan_attr, Lag._VLAN,
                                                                vlan)
                        vlan_xpath = "{0}/vlan[{1}]".format(ether_xpath, idx + 1)
                        imports.append(self._set_multivalue(vlan_xpath, attr_values.ixn_attribute,
                                                            attr_values.config_value))
        self._import(imports)
        ...
        for name, ports in self._lag_ports.items():
            ...
            for port in ports:
                ...
                if choice is None:
                    ...
                    raise Exception("Please configure same protocol "
            ...
            if choice == 'lacp':
                ...
                for lacp_attr in Lag._LACP:
                    attr_values = self._configure_attribute(lacp_attr, Lag._LACP,
                                                            protocols, choice)
                ...
                for static_attr in Lag._STATIC:
                    attr_values = self._configure_attribute(static_attr, Lag._STATIC,
                                                            protocols, choice)
                    imports.append(self._set_multivalue(static_xpath, attr_values.ixn_attribute,
                                                        attr_values.config_value))
